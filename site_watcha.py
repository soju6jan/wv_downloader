...
from .site_base import SiteBase, d, logger, package_name, ModelSetting, P, path_data, ToolBaseFile, webdriver, WebDriverWait, EC, By, Keys
        ...
        try:
            for item in self.data['har']['log']['entries']:
                if item['request']['method'] == 'GET' and item['request']['url'].find('%s.json' % self.code) != -1:
                    ...
                    ToolBaseFile.write_json(self.meta['source'], os.path.join(self.temp_dir, '{code}.meta.json'.format(code=self.code)))
                """
